...
The Extension reads limited content and interface elements on `https://adamson.blackboard.com/`, such as course titles, section numbers, navigation items, and page structure. This processing is necessary to create course banners, improve the course grid, build the side navigation, apply reading themes, and provide keyboard navigation.
...
The Extension stores the user's selected reading theme and customized keyboard shortcuts using the `chrome.storage.sync` API. If Chrome Sync is enabled, Google Chrome may synchronize these preferences between browsers signed in to the same Google account. The developer does not receive or have access to these preferences.
...
- Names, student numbers, email addresses, or other identity information
...
- Browsing history or activity outside Adamson University's Blackboard website
...
The Extension also provides optional links to Outlook, AdU Live, the Adamson University academic calendar, GitHub, and other external pages. These websites operate independently and are governed by their own privacy policies. The Extension does not send Blackboard page content to these websites.
